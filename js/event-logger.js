...
class EventLogger {
  ...
  setupErrorHandlers() {
    ...
    console.error = (...args) => {
      // Sanitize arguments to remove non-serializable objects (like functions)
      ...
      window.trackDownloadProgress = function (fileName, url, progress) {
        ...
        if (window.eventLogger) {
          if (progress === 0) {
            ...
            window.eventLogger.logContentDownload("completed", fileName, {
              url,
            });
          } else {
            window.eventLogger.logContentDownload("progress", fileName, {
              url,
              progress,
            });
          }
