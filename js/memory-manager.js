...
class MemoryManager {
  ...
  createManagedConsole() {
    ...
    console.log = (...args) => {
      window.CONSOLE_LOG_COUNT = (window.CONSOLE_LOG_COUNT || 0) + 1;
      if (window.CONSOLE_LOG_COUNT <= (window.MAX_CONSOLE_LOGS || 50)) {
        this.originalConsole.log(...args);
      } else if (
        window.CONSOLE_LOG_COUNT ===
        (window.MAX_CONSOLE_LOGS || 50) + 1
      ) {
        this.originalConsole.warn(
          "🚨 Console log limit reached. Suppressing further logs to save memory."
        );
      }
    };

    // Keep error and warn always functional
    console.error = (...args) => {
    ...
    console.info = (...args) => {
      if ((window.CONSOLE_LOG_COUNT || 0) <= (window.MAX_CONSOLE_LOGS || 50)) {
  ...
  rotateLogs() {
    ...
    const threshold = Math.floor(
      maxEntries * (window.LOG_CLEANUP_THRESHOLD / 100)
    );
  ...
  checkMemoryUsage() {
    ...
    this.addInfoLog(
      `📊 Memory: Logs=${stats.totalLogs}, Timeouts=${stats.timeouts}, Intervals=${stats.intervals}, Listeners=${stats.eventListeners}`
    );
